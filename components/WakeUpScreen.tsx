...
export const WakeUpScreen: React.FC<WakeUpScreenProps> = ({ 
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col items-center justify-center p-6 bg-black/60 backdrop-blur-xl transition-opacity duration-1000 ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="flex flex-col items-center max-w-4xl text-center space-y-10 w-full">
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <div className="flex items-center gap-2 bg-white/10 p-1 rounded-full backdrop-blur-sm border border-white/10">
            {[5, 10, 15].map(d => (
              <button
                key={d}
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 mt-2">
            <button
              ...
              className="group relative flex items-center gap-3 px-8 py-4 bg-white/10 hover:bg-white/20 rounded-full border border-white/20 transition-all duration-300 hover:scale-105 active:scale-95 shadow-lg backdrop-blur-md w-full md:w-auto justify-center"
            >
